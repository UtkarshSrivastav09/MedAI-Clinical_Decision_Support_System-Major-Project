  ...
  return (
    <div className="animate-fade-in dashboard-container">
      ...
      <div className="glass-card directory-panel">
        <div className="directory-tools" style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
           ...
           <button onClick={downloadCSV} className="action-btn" style={{padding: '10px 24px', fontSize: '0.9rem', display: 'flex', alignItems: 'center', gap: '8px'}}>
              ...
              filteredPatients.map((p) => {
                  ...
                  return (
                    <React.Fragment key={p.id}>
                      ...
                      >
                        <td style={{verticalAlign: 'middle'}}>{isExpanded ? <ChevronUp size={16}/> : <ChevronDown size={16}/>} #{p.id}</td>
                        <td style={{verticalAlign: 'middle'}}>{new Date(p.date).toLocaleString()}</td>
                        ...
                        <td style={{verticalAlign: 'middle'}}>
                          <div style={{display: 'flex', alignItems: 'center', gap: '8px', fontSize: '0.85rem'}}>
                            <Activity size={14} color="var(--accent-cyan)" /> {p.report?.clinical_assessment?.severity || 'Low'}
                          </div>
                        </td>
                        <td style={{verticalAlign: 'middle'}}>
